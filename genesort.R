...
setwd('~/Desktop/Palicourea/Manuscript/genesort/')

#INPUT: fill this with file names
alignment <- '77_loci.fasta'
partition <- '77_partitions.txt'
species_tree <- 'ASTRAL_810_allexons-BS10_pp_rooted.tre'
gene_trees <- '77_gene_trees.tre'
...
ingroup <- c('Psy_jinotegensis_Stevens33549', 'Pal_suerrensis_Stevens36741')
...
for(i in 1:length(gene_trees)) {
  ...
  if(topological_similarity) {
    robinson_sim[i] <- 1 - suppressMessages(RF.dist(this_species_tree, tree, normalize = TRUE, check.labels = F))
  }
...
variables_to_use <- which(colnames(variables) %in% c('root_tip_var', 'saturation', 'missing','rate','tree_length','treeness','av_patristic', 'RCFV', 
                                                     'length','occupancy','variable_sites', 'average_BS_support', 'robinson_sim'))
